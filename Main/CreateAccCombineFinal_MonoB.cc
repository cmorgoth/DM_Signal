...
int main(){
  ...
  TH1F* h_acc_pdf_CTEQ[8][45];
  TH1F* h_acc_pdf_MRST[8][31];
  TH1F* h_acc_pdf_NNPDF[8][101];
  
  TH1F* h_acc_pdf_CTEQ_up[8];
  TH1F* h_acc_pdf_MRST_up[8];
  TH1F* h_acc_pdf_NNPDF_up[8];
  
  TH1F* h_acc_pdf_CTEQ_down[8];
  TH1F* h_acc_pdf_MRST_down[8];
  TH1F* h_acc_pdf_NNPDF_down[8];

  TH1F* h_acc_CENTRAL[8];
  
  TH1F* h_acc_PDF_up[8];
  TH1F* h_acc_PDF_down[8];
  
  TH1F* h_acc_ISR_up[8];
  TH1F* h_acc_ISR_down[8];
  
  TH1F* h_acc_JES_up[8];
  TH1F* h_acc_JES_down[8];
  ...
  if (mfile0.is_open()){
	...
	for(int l = 0; l < 101; l++){
      ...
      sn = TString("_acc_cat");
      h_acc[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      for(int l = 0; l < 45; l++){
	sn = TString(Form("_acc_pdfCTEQ_index%d",l));
	h_acc_pdf_CTEQ[xs_counter][l] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      }
      sn = TString("_acc_pdfCTEQ_up");
      h_acc_pdf_CTEQ_up[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_pdfCTEQ_down");
      h_acc_pdf_CTEQ_down[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      
      for(int l = 0; l < 31; l++){
	sn = TString(Form("_acc_pdfMRST_index%d", l));
	h_acc_pdf_MRST[xs_counter][l] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      }
      sn = TString("_acc_pdfMRST_up");
      h_acc_pdf_MRST_up[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_pdfMRST_down");
      h_acc_pdf_MRST_down[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      
      for(int l = 0; l < 101; l++){
	sn = TString(Form("_acc_pdfNNPDF_index%d", l));
	h_acc_pdf_NNPDF[xs_counter][l] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      }
      sn = TString("_acc_pdfNNPDF_up");
      h_acc_pdf_NNPDF_up[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_pdfNNPDF_down");
      h_acc_pdf_NNPDF_down[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      
      sn = TString("_acc_CENTRAL");
      h_acc_CENTRAL[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_TOTAL_PDF_up");
      h_acc_PDF_up[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_TOTAL_PDF_down");
      h_acc_PDF_down[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      
      //JES
      sn = TString("_acc_JES_up");
      h_acc_JES_up[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_JES_down");
      h_acc_JES_down[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      
      //ISR
      sn = TString("_acc_ISR_up");
      h_acc_ISR_up[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
      sn = TString("_acc_ISR_down");
      h_acc_ISR_down[xs_counter] = new TH1F(DM_Name+sn, DM_Name+sn, nBins, R2Bin);
	...
	if(mr[2] >= 200.0 && rsq[2] >= 0.5 && btag == 0 && box == 0 && fabs(Dphi) < 2.5){
	  ...
	  if(mr[2] > 200.0 && mr[2] <= 3500.0){
	    ...
	    h_acc[xs_counter]->Fill(rsq[2], ff_w/Gen_Evts_isr);
	    h_acc_ISR_up[xs_counter]->Fill(rsq[2], isr_up);
	    h_acc_ISR_down[xs_counter]->Fill(rsq[2], isr_down);
	    for(int l = 0; l < nCTEQ66; l++)h_acc_pdf_CTEQ[xs_counter][l]->Fill(rsq[2], ff_w*wCTEQ66[l]/N_pdf_CTEQ66_isr_Tot[l]);
	    for(int l = 0; l < nMRST2006NNLO; l++)h_acc_pdf_MRST[xs_counter][l]->Fill(rsq[2], ff_w*wMRST2006NNLO[l]/N_pdf_MRST2006NNLO_isr_Tot[l]);
	    for(int l = 0; l < nNNPDF10100; l++)h_acc_pdf_NNPDF[xs_counter][l]->Fill(rsq[2], ff_w*wNNPDF10100[l]/N_pdf_NNPDF10100_isr_Tot[l]);
	  }
	  N_passed += hlt_w*pu_w;
	...
	if(mr_up[2] >= 200.0 && rsq_up[2] >= 0.5 && btag == 0 && box == 0 && fabs(Dphi) < 2.5){
	  ...
	  if(mr_up[2] > 200.0 && mr_up[2] <= 3500.0 ){
	    ...
	    h_acc_JES_up[xs_counter]->Fill(rsq_up[2], weight);
	  }
	}
	...
	if(mr_down[2] >= 200.0 && rsq_down[2] >= 0.5 && btag == 0 && box == 0 && fabs(Dphi) < 2.5){
	  ...
	  if(mr_down[2] > 200.0 && mr_down[2] <= 300.0 ){
	    ...
	    h_acc_JES_down[xs_counter]->Fill(rsq_down[2], weight);
	  }
	}
	...
	for(int bin = 1; bin <= h_acc_pdf_CTEQ[xs_counter][0]->GetNbinsX(); bin++){
	  double wplus = 0.0;
	  double wminus = 0.0;
	  for(int l = 0; l < npair; l++){
	    double wa = 0.0;
	    if(h_acc_pdf_CTEQ[xs_counter][2*l+1]->GetBinContent(bin)){
	      wa = h_acc_pdf_CTEQ[xs_counter][2*l+1]->GetBinContent(bin)/h_acc_pdf_CTEQ[xs_counter][0]->GetBinContent(bin) - 1.0;
	    }
	    double wb = 0.0;
	    if(h_acc_pdf_CTEQ[xs_counter][2*l+2]->GetBinContent(bin)){
	      wb = h_acc_pdf_CTEQ[xs_counter][2*l+2]->GetBinContent(bin)/h_acc_pdf_CTEQ[xs_counter][0]->GetBinContent(bin) - 1.0;
	    }
	  ...
	  wplus = sqrt(wplus);
	  wminus = sqrt(wminus);
	  std::cout << " Bin: " << bin << " DeltaUp: " << wplus*100.0 << " % " <<
	    " DeltaDown: " << wminus*100.0 << " %"<< std::endl;
	  double acc_up = h_acc_pdf_CTEQ[xs_counter][0]->GetBinContent(bin)+wplus*h_acc_pdf_CTEQ[xs_counter][0]->GetBinContent(bin);
	  double acc_down = h_acc_pdf_CTEQ[xs_counter][0]->GetBinContent(bin)-wminus*h_acc_pdf_CTEQ[xs_counter][0]->GetBinContent(bin);
	  h_acc_pdf_CTEQ_up[xs_counter]->SetBinContent(bin, acc_up);
	  h_acc_pdf_CTEQ_down[xs_counter]->SetBinContent(bin, acc_down);
	}
	
	
	//MRST Error Calculation
	npair = 15;
	for(int bin = 1; bin <= h_acc_pdf_MRST[xs_counter][0]->GetNbinsX(); bin++){
	  double wplus = 0.0;
	  double wminus = 0.0;
	  for(int l = 0; l < npair; l++){
	    double wa = 0.0;
	    if(h_acc_pdf_MRST[xs_counter][2*l+1]->GetBinContent(bin)){
	      wa = h_acc_pdf_MRST[xs_counter][2*l+1]->GetBinContent(bin)/h_acc_pdf_MRST[xs_counter][0]->GetBinContent(bin) - 1.0;
	    }
	    double wb = 0.0;
	    if(h_acc_pdf_MRST[xs_counter][2*l+2]->GetBinContent(bin)){
	      wb = h_acc_pdf_MRST[xs_counter][2*l+2]->GetBinContent(bin)/h_acc_pdf_MRST[xs_counter][0]->GetBinContent(bin) - 1.0;
	    }
	  ...
	  std::cout << " Bin: " << bin << " DeltaUp: " << wplus*100.0 << " % " <<
	    " DeltaDown: " << wminus*100.0 << " %"<< std::endl;
	  double acc_up = h_acc_pdf_MRST[xs_counter][0]->GetBinContent(bin)+wplus*h_acc_pdf_MRST[xs_counter][0]->GetBinContent(bin);
	  double acc_down = h_acc_pdf_MRST[xs_counter][0]->GetBinContent(bin)-wminus*h_acc_pdf_MRST[xs_counter][0]->GetBinContent(bin);
	  h_acc_pdf_MRST_up[xs_counter]->SetBinContent(bin, acc_up);
	  h_acc_pdf_MRST_down[xs_counter]->SetBinContent(bin, acc_down);
	}
	
	for(int bin = 1; bin <= h_acc_pdf_MRST[xs_counter][0]->GetNbinsX(); bin++){
	  double cteqUp = h_acc_pdf_CTEQ_up[xs_counter]->GetBinContent(bin);
	  double cteqDown = h_acc_pdf_CTEQ_down[xs_counter]->GetBinContent(bin);
	  double mrstUp = h_acc_pdf_MRST_up[xs_counter]->GetBinContent(bin);
	  double mrstDown = h_acc_pdf_MRST_down[xs_counter]->GetBinContent(bin);
	  ...
	  h_acc_CENTRAL[xs_counter]->SetBinContent(bin, (MIN+MAX)/2.0);
	  h_acc_PDF_up[xs_counter]->SetBinContent(bin, (MAX - (MIN+MAX)/2.0)/((MIN+MAX)/2.0));
	  h_acc_PDF_down[xs_counter]->SetBinContent(bin, ((MIN+MAX)/2.0 - MIN)/((MIN+MAX)/2.0));
	}    
	
	TFile* fo = new TFile("AccBIS/"+s1+"_Acc.root", "RECREATE");
	TString dummy;
	/*
	h_acc[xs_counter]->Write();
	  
	h_acc_pdf_CTEQ[xs_counter][0][i]->Write();
	h_acc_pdf_CTEQ_up[xs_counter]->Write();
	h_acc_pdf_CTEQ_down[xs_counter]->Write();
	  
	h_acc_pdf_MRST[xs_counter][0][i]->Write();
	h_acc_pdf_MRST_up[xs_counter]->Write();
	h_acc_pdf_MRST_down[xs_counter]->Write();
	  
	h_acc_CENTRAL[xs_counter]->Write();
	*/
	dummy = "PDF_SYS";
	h_acc_PDF_up[xs_counter]->Write(dummy);
	/*
	  h_acc_PDF_down[xs_counter]->Write();
	  
	  h_acc_JES_up[xs_counter]->Write();
	  h_acc_JES_down[xs_counter]->Write();
	  
	  h_acc_ISR_up[xs_counter]->Write();
	  h_acc_ISR_down[xs_counter]->Write();
	  */
	fo->Close();
      }
